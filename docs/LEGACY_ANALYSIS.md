...
| D9 | R4 | `Module2.bas` lines 47–61 (`CoutNC`) — return type is `Long` (`Function CoutNC(r) As Long`), meaning VBA converts the computed cost to a whole number via `CLng()` before returning | `CoutNC` rounds each record's CNQ contribution to the nearest whole euro (VBA banker's rounding) before it is added to the accumulator in `CalcHebdo` and `CalcMensuel`. The scrap and rework cost formulae produce fractional values whenever `rework_hours_per_part × 45` is not a whole number (e.g. D01: 0.25 h × 45 = 11.25 €; D02: 0.5 h × 45 = 22.5 €). | R4 defines CNQ as a sum of per-record costs (`qty × cost`), with no per-record rounding step stated. Rounding, if needed at all, applies only to the final reported total. | **0 CSV mismatches** in current dataset — `tools/rules_check.py` v4 also uses `round(cnq, 0)` on the per-record accumulation result, inadvertently reproducing the same error. The accumulated error on the full dataset is −9 € (per-record-rounded sum = 108 067 €; exact-then-round = 108 076 €), affecting 53 records with fractional per-record costs. |
...
| D9 | Per-record CNQ rounding (`CLng()` in `CoutNC`) | 0 (checker blind spot) | **Fix**: accumulate exact fractional costs; round only the final weekly and monthly totals. Net error on current data = −9 €. |
